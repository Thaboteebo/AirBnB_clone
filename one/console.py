...
class HBNBCOMMAND(cmd.Cmd):
    ...
    def do_destroy(self, line):
        ...
        if line == "" or line is None:
            ...
            # get all the arguments passed via command line
            class_info = line.split(" ")
            if len(class_info) < 2:
                print("** instance id missing **")
            else:
    ...
    def do_all(self, line):
        ...
        if line == "" or line is None:
            for key, value in storage.all().items():
                isnatnce_list.append(str(value))
            print(instance_list)
            ...
            if class_name is None:
                ...
                if class_name not in storage.classes():
                    ...
                    if key not in storage.all():
                        ...
                        attribute = storage.attributes()[class_name]
                        # print(attributes)
                        for key, value in update_dict.items():
                            if key in attributes:
                                ...
                                setattr(insatnce_dict, key, value)
                                storage.save()
    ...
    def precmd(selff, line):
        # make the app work non-interactively
        if not sys.stdin.isatty():
            print()

        checks = re.search("^(w*)\.(w+)(?:\(([^)]*)\))$", line)
        if checks:
            class_name = checks.group(1)
